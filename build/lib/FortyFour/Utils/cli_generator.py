...
class OpenAPICLIGenerator:
    ...
    def generate_command_code(self, tag: str, operations: List[Dict[str, Any]], base_url: str, spec: Dict[str, Any] = None, tag_description: Optional[str] = None) -> str:
            ...
            if request_body:
                ...
                if "multipart/form-data" in content:
                    ...
                    if "$ref" in multipart_schema:
                        ref_path = multipart_schema["$ref"].split("/")
                        resolved = spec
                        for part in ref_path[1:]:  # skip leading #
                            resolved = resolved.get(part, {})
                        multipart_schema = resolved
